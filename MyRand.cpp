...
unsigned int MyRand::operator()(unsigned int lRand)
{
	...
	{
		resRand = (((eGlobRand + dGlobRand) % (iterGlobRand * 3)) % 2);
		previousGlobRand = resRand;
		iterGlobRand += 77;
		dGlobRand += iterGlobRand;
	...
	{
		resRand = ((eGlobRand + (iterGlobRand * 3 + previous2GlobRand)) % lRand) + 1;
		if (previousGlobRand == resRand)
		{
			bool done = false;
			while (done == false)
	...
	{
		resRand = ((((eGlobRand % lRand) * (eGlobRand % lRand)* (eGlobRand % lRand)) + (iterGlobRand + previousGlobRand + previous2GlobRand + previous3GlobRand)) % lRand)+ 1;
		if (previousGlobRand == resRand)
		{
			bool done = false;
			while (done == false)
			{
				resRand = (((dGlobRand % lRand)* (dGlobRand % lRand)* (dGlobRand % lRand) + (iterGlobRand + (previousGlobRand + previous2GlobRand + previous3GlobRand))) % lRand)+1;
				if (resRand != previousGlobRand) done = true;
		...
		if (previous2GlobRand == resRand)
		{
			...
			while (done == false)
			{
				resRand = ((((cGlobRand % lRand) * (cGlobRand % lRand) + iterGlobRand) + (previousGlobRand + previous2GlobRand + previous3GlobRand)) % lRand)+1;
				if (resRand != previous2GlobRand) done = true;
		...
		if (previous3GlobRand == resRand)
		{
			...
			while (done == false)
			{
				resRand = ((bGlobRand % lRand + iterGlobRand + (previousGlobRand + previous2GlobRand + previous3GlobRand)) % lRand)+1;
		...
		iterGlobRand += 77;
		bGlobRand += iterGlobRand;
		cGlobRand += iterGlobRand;
		dGlobRand += iterGlobRand;
		eGlobRand += iterGlobRand;
		while(resRand > lRand)
		{
